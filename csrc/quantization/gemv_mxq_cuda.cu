...
__global__ void gemv_mxq_kernel_g16_v0(
    ...
    const int loop_num = int((IC+2047) / 2048);
    ...
    for (int packed_group_idx = 0; packed_group_idx < loop_num; packed_group_idx++)
    {
...
#pragma unroll
        ...
        { // Get first 3 uint32 packed weights(total 3*16*2b)
            ...
            // half scaling_factor_2nd = scales_2nd[oc_idx / second_order_group_size * scale_w + packed_group_idx * scale_w/ loop_num + threadIdx.x * 3 + ii];
            half scaling_factor_2nd = scales_2nd[oc_idx / second_order_group_size * scale_w + threadIdx.x * 3 + ii];
            float scaling_factor = __half2float(scaling_factor_2nd) * (current_1st_scales - current_2nd_zeros);
            // float scaling_factor = (current_1st_scales - current_2nd_zeros);
...
torch::Tensor gemv_mxq_forward_cuda(
    ...
    int group_size)
{
    int num_batchsize = _in_feats.size(0); // bs
    int num_in_feats = _in_feats.size(1); // seqlen
    ...
    // kernel is [OC, IC]
    at::Tensor _out_feats = torch::empty({num_batchsize, num_in_feats, _kernel.size(0)}, options);
    ...
    if (group_size == 16 & num_out_feats == 1)
    {
